...
你的任务是严格审核以下滚动章节规划(chapter_plan)条目，确保其结构合法、承接自然、节奏合理、字段可用。

## 待审核内容(严格 JSON 数组)
{draft}

## 审核要点
1. 【结构合法】是否为合法 JSON 数组、无 markdown 围栏、无前置解释？每条对象是否严格包含且仅包含 4 个字段 `chapter / purpose / key_turn / ending_hook`？字段类型是否正确(chapter=int,其余=str)？
2. 【章号连续】章号是否严格连续升序、无跳号无重复？是否与「本次任务范围」(生成 prompt 中标明的起止章号)完全一致？是否与「已锁定的历史条目」章号无重叠？
3. 【大纲对齐】各章的 `purpose` 与 `key_turn` 是否落在整书大纲对应阶段(起承转合/四卷式)?是否提前引爆了应属于后期的核心悬念或反派揭底?
4. 【承接自然】滚动重规划场景下,首条(第 done+1 章)是否与「已写完章节速览」的伏笔、人物状态、势力格局、情绪走向自然咬合?有无与前文矛盾的设定或人物行为?
5. 【转折分布】强转折/爆发/反转类的高密度章节数量是否合理(占比约 8%-25%)?是否存在连续 3 章以上硬转折(节奏窒息)?是否存在超过 5 章连续平推(节奏拖沓)?
6. 【字段可用】`purpose / key_turn / ending_hook` 是否具体到「谁做了什么/看到了什么/说了什么」这种可落地的具体事件?是否有「进入下一战」「揭开真相」「继续推进」等空洞占位?
7. 【字数硬限】`purpose ≤ 40 汉字`、`key_turn ≤ 40 汉字`、`ending_hook ≤ 30 汉字`是否全部达标?

## 输出规范
如内容合格，只输出：无问题
否则逐条指出具体问题（点明是哪一章、哪个字段），并给出可执行的修改建议。"""

...
请直接输出 JSON 对象。"""
    ...
    def overall_outline_prompt(self, total_word_count: str) -> str:
        """生成全书 Synopsis 式战略骨架（故事内核/主线动力/暗线悬念承诺/人物弧光/结局锚点五小节）。

        不再要求起承转合四段作文——叙事节拍下沉到卷级（volumes 已有卷内起承转合）；
        全书级只锁「方向 + 悬念承诺 + 结局锚点」，具体事件由 volumes / chapter_plan 滚动落地。
        """
...
整体硬性标准：主角视角清晰，悬念承诺可追踪、不空洞，逻辑自洽，框架具备强延展性、可支撑长篇连载滚动展开。{focus}
{_FOUNDATION_RIGOR}
...
5. 后续每卷从前一卷 setup_for_next 自然承接，整体沿 overall_outline 的主线动力推进、按悬念承诺的兑现时机逐层揭线，不重复不倒退。
...
3. 激活卷必须**自然承接上一卷卷尾的钩子/悬念**，沿 overall_outline 的主线动力继续推进、按悬念承诺的兑现时机揭线，**不重复已写内容、不倒退**。

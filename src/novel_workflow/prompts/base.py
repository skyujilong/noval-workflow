...
_FOUNDATION_RIGOR = """
【底层设定硬约束（必须满足，否则重写）】
- 因果闭环：每一条设定都要能自圆其说——写清"为什么会这样"，不留悬空前提，不靠"天生如此 / 因为剧情需要"搪塞；凡引入一个设定，其成因、代价、边界须在本设定或前置上下文中可追溯。
...
- 强延展、留白：只搭骨架与底层逻辑，为下游大纲 / 人物预留扩展空间，不写死一次性细节。

【输出前自检（逐条自问，全部通过才输出，否则改到通过）】
1. 是否有任一条设定无法回答"为什么成立"、只是凭空规定？——若有，补足因果或删除。
...
- **能力/底牌契约（硬绑世界观）**：每位核心角色的能力必须归属【世界观设定】的规则 / 力量体系，**禁止体系外凭空能力**；须写明【初始锚点】（开篇实力起点）与【四卷成长天花板】（卷一→卷四各一句，只写能力上限走势，不写具体招式）；若有隐藏杀手锏（底牌），须写明触发条件与反噬 / 代价，作为暗线素材，不得提前于设定卷次解锁。此契约为后续「阶段固化数据」台账的战力校验红线。
...
5. 每位角色的能力是否都未越出【世界观设定】规则体系与其【四卷成长天花板】？"""

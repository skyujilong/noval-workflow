...
请直接输出冲突设计内容，不需要标题。"""
    ...
    def character_cards_prompt(self) -> str:
        ...
        省一跑 LLM。深层设计承载在 canon 字段里：四卷弧光核心角色写；双层人设/底牌契约按 role
...
为全书一次性生成可对接四卷式大纲、适配明暗双线、分层立体的**全套核心人物结构化卡**，直接输出 JSON（不写散文档案）。
...
```
{{"name": "角色名（唯一，作主键）", "type": "人物", "aliases": ["别称/绰号，无则空数组"], "summary": "一句话定位（≤30字）", "first_appear_chapter": 1, "role": "只能填:主角/主要配角/功能性反派/根源反派/感情线角色/次要角色", "appearance": "体貌基线（身形/气质/年龄段/大致长相,供正文外貌一致性）+ 可选1个识别特征,≤60字", "speech_style": "说话风格/口吻/温度（健谈/毒舌/寡言/温和/跳脱…,≤30字）", "personality": "【表层公开人设】读者视角性格底色", "abilities": "能力底牌一句话摘要（须落【力量体系】层级/流派）", "hidden_persona": "【深层隐藏人设·条件字段】仅主角/根源反派/确有反转设计的关键角色写:暗线秘密/异常/隐藏能力/立场偏差,与表层不冲突、可后期反转;普通配角/功能角色留空,不强凑", "arc_trajectory": "四卷弧光:卷一→卷四心性/立场/羁绊/认知迭代大势(只写大势不填情节);反派写阶段作用+闭环退场", "ability_contract": "【条件字段】仅战力相关的关键角色/确有需要才写:初始锚点(开篇实力对应力量体系哪层)+四卷成长天花板(卷一→卷四能力上限走势)+隐藏杀手锏(触发与反噬,不提前解锁);非战斗/次要/纯功能角色留空或只写简单 abilities", "motivation": "开篇当前动机/目标", "current_state": "开篇处境:位置/情绪/状态（≤30字）", "relations": "与主角/他人关系(围绕主角:情感位/信任位/冲突位/对立位/功能位)"}}
```
...
- **严格绑定四卷成长**：`arc_trajectory` 只写心性/立场/羁绊/认知的迭代大势，不填充具体情节/互动/台词。
- **能力落体系 + 底牌契约按需**：`abilities` 必须归属【力量体系】层级/流派，禁止体系外凭空能力（全员适用）。`ability_contract`（初始锚点+四卷天花板+隐藏杀手锏）仅对战力相关的关键角色写，非战斗/次要/纯功能角色留空——不给官僚/文职等角色硬塞「隐藏杀手锏」。写了契约的，隐藏杀手锏须写明触发与反噬、作为暗线素材不得提前解锁；此契约为「阶段固化数据」台账的战力校验红线。
...
3. 每张卡 `role` 是否为六枚举之一？
4. 各角色 `abilities` 是否归属【力量体系】？写了 `ability_contract` 的角色初始锚点与四卷天花板是否落在境界阶梯上、非战斗角色是否未硬塞契约？

...
- **题材**: 玄幻
- **节点**: `volumes_rolling` (prepare)
- **fixture 说明**: fixture: 已有 1 激活卷,触发滚动重规划分支
- **system 长度**: 483 字
- **context 长度**: 2723 字
- **task 长度**: 1117 字
...
【本次任务】
本书正在连载，现需重新规划**从第 2 卷起的后续 3 卷**（第 2 卷是即将展开的激活卷，第 3-4 卷是前瞻草稿），返回严格 JSON 对象。
...
- 第 1 卷《第一卷 · 少年入局》(第 1-30 章)：山村少年被卷入旧事，卷末踏入更大的舞台 | 卷尾钩：母亲遗物半张地图指向禁地入口

# 当前进度
- 全书已写完 3 章
- 上一卷卷尾埋的钩：母亲遗物半张地图指向禁地入口
- 第 2 卷将从第 31 章开始（系统已锁定，你无需输出章号）

# 硬约束
1. 输出**纯 JSON 对象**（以 { 开头、} 结尾），不要 markdown 围栏，不要解释文字。
2. 对象仅含 `volumes` 数组，长度 3：第 1 项 = 第 2 卷（激活卷，4 字段含 chapters）；其余 = 前瞻草稿卷（3 字段，无 chapters）。
...
# 输出样例（仅示字段格式；实际 volumes 长度以硬约束 3 为准）
{"volumes": [{"title": "第 2 卷 · 风起", "summary": "承接上一卷悬念，主角踏入新舞台，卷末揭开一层真相", "setup_for_next": "盟友真实身份浮出水面", "chapters": 32}, {"title": "第 3 卷 · 潮涌", "summary": "新势力入局，矛盾升级", "setup_for_next": "关键抉择迫近"}]}

...
- **题材**: 科幻
- **节点**: `volumes` (prepare)
- **fixture 说明**: fixture: volumes 清空(触发首次分卷分支)
- **system 长度**: 483 字
- **context 长度**: 2723 字
- **task 长度**: 1276 字
...
【本次任务】
任务：为本书规划开篇的前 3 卷，返回严格 JSON 对象。
第 1 卷是**要立即展开写作的激活卷**，第 2-3 卷是**前瞻草稿卷**（只给方向、不写细节，用于规划当前卷时提供中期地图）。
...
2. 对象仅含一个字段 `volumes`：一个长度为 3 的数组。
   - 激活卷（数组第 1 项）含 4 字段：

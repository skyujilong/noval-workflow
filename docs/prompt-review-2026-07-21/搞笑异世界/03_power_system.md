...
- **题材**: 搞笑异世界
- **节点**: `power_system` (prepare)
- **fixture 说明**: fixture: has_power_system=True,已有 world_building
- **system 长度**: 548 字
- **context 长度**: 2726 字
- **task 长度**: 1323 字
...
- 字数：400-800字
- 题材聚焦：保留冒险者等级 / 职业 / 技能 / MP / 幸运值 / 每日限用次数 / 职业禁用限制等 RPG 规则，把它们当认真的世界规则用（例如：「神职系转职后禁用杀伤武器」类职业限制、「防御向骑士攻击命中率致命偏低」类硬缺陷、「盗贼技能『偷窃』的成功率与幸运值挂钩」等）。**明确 ban 修炼境界式的线性成长阶梯**——主角的等级 / 技能列表允许缓慢增加，但战力不许出现「越阶碾压 / 血脉觉醒 / 大境界突破」式跃迁；配角能力上限须与其致命废柴属性绑定，**能力越强，废柴限制越硬**。

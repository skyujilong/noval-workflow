...
刻意丢弃 context_prompt（唯一承载【世界观设定】的 L2），导致固化数据的等级/资源等硬性数值
无法与世界观力量/等级体系对齐。（人物动态 status/relations 两条腿已并入 CharacterCard。）

修复：_prepare_* 走 build_prepare_fields，身份=数据维护员进 system_prompt（L1），设定
经 build_foundation_context(include_identity=False) 进 context_prompt（L2）；generate/
llm_self_review 用 render_user 把 L2 拼进 user。本测试锁定：
  1) build_foundation_context(include_identity=False) 输出无创作者身份前缀、但含世界观；
  2) 快照 generate 的 system 含「数据维护员」身份、user 含世界观（L2 经 render_user 拼入）；
...
class _FakeLLM:
    ...
    def invoke(self, messages):
        ...
        return AIMessage(
            content="无问题" if self.label.startswith("self_review:") else "占位草稿"
        )
...
def test_settings_only_context_has_worldview_without_creative_identity():
    ...
    settings_only = build_foundation_context(
        state, exclude_snapshots=True, include_identity=False
    )
    assert "【世界观设定】" in settings_only
    assert WORLD in settings_only
    assert _IDENTITY_PREAMBLE not in settings_only, (
        "include_identity=False 不应含创作者身份前缀"
    )

    with_identity = build_foundation_context(
        state, exclude_snapshots=True
    )  # 默认 include_identity=True
    assert _IDENTITY_PREAMBLE in with_identity, (
        "默认应保留创作者身份前缀（非快照流程不受影响）"
    )
...
def test_snapshot_generate_injects_worldview(monkeypatch):
    ...
    # 新架构：system_prompt 含数据维护员身份（L1），context_prompt 含世界观（L2），
    # generate 用 render_user 把 L2 拼进 user。prepare_initial_status 已用此模式，
    # 这里直接复用它产出真实三字段，验证 generate 拼装。
    state_full = NovelState(genre="玄幻", world_building=WORLD)
    prepared = prepare_initial_status(state_full)
    ...
    user_text = str(gen[0][1].content)  # messages[1] 为 HumanMessage（含 L2 资料）
    assert "数据维护员" in sys_text, "system 应含数据维护员身份"
    assert WORLD in user_text, "快照生成的 user（L2 经 render_user）必须注入世界观"
    assert WORLD not in sys_text, "世界观在 L2/user，不应在 system（避免稀释主线）"
...
def test_prepare_initial_status_reuses_phase_summary_snapshot_wiring():
    ...
    assert out["review_type"] == "phase_summary", (
        "复用 phase_summary 以继承数据维护员身份/审核/中断"
    )
    assert "人物初始基线" in out["task_prompt"], (
        "task_prompt 应来自 initial_status_prompt"
    )
    sc = out["context_prompt"]
    assert WORLD in sc and "【人物档案】" in sc, (
        "context_prompt 须含世界观与人物档案（初始化数据来源）"
    )
...
def test_save_initial_status_writes_phase_summary_field():
    ...
    assert save_initial_status(state) == {
        "phase_summary": "角色：叶凡\n【核心能力】炼气一层"
    }
...
def test_seeded_baseline_is_inherited_as_prev_on_first_batch():
    ...
    assert "完整保留上次快照" in prompt, (
        "prev 非空须触发 carry-over（继承而非重造基线）"
    )
